...
#define SOURCE_MODE xal::SourceMode::Ram
#else
#define SOURCE_MODE xal::SourceMode::Disk
#endif
...
void _test_fadein(xal::Player* player)
{
	...
	xal::Player* p1 = xal::manager->createPlayer(S_WIND);
	p1->play(1.0f);
	for_iter (i, 0, 20)
	{
		hthread::sleep(100);
...
void _test_complex_handler(xal::Player* player)
{
	...
	xal::Player* temp;
	xal::Player* p1 = xal::manager->createPlayer(S_WIND);
	xal::Player* p2 = xal::manager->createPlayer(S_WIND_2);
	p1->play();
	p2->play();
	p2->pause();
	for_iter (i, 0, 50)
	...
	p1->stop();
	p2->stop();
	xal::manager->destroyPlayer(p1);
	xal::manager->destroyPlayer(p2);
	xal::manager->update(0.1f);
}
...
void _test_memory_management(xal::Player* player)
{
	...
	xal::manager->setIdlePlayerUnloadTime(0.5f);
	player->play();
	hthread::sleep(200);
	xal::manager->update(0.2f);
	player->pause();
	...
	xal::manager->clearMemory();
	hthread::sleep(1000);
	xal::manager->update(0.5f);
	p1->stop();
	player->stop();
...
#endif
{
	...
	xal::init(xal::AudioSystemType::Default, hwnd, false);
#else
	xal::init(xal::AudioSystemType::Default, hwnd, true, 0.01f);
#endif
#ifdef _USE_MEMORY_MANAGEMENT
	xal::manager->createCategory("sound", xal::BufferMode::Managed, SOURCE_MODE);
#endif
	harray<hstr> files = xal::manager->createSoundsFromPath(RESOURCE_PATH, "sound", "");
#ifndef _USE_LINKS
#ifndef _USE_STREAM
	xal::manager->createCategory("streamable", xal::BufferMode::Managed, SOURCE_MODE);
#else
	xal::manager->createCategory("streamable", xal::BufferMode::Streamed, SOURCE_MODE);
#endif
	files += xal::manager->createSoundsFromPath(RESOURCE_PATH "streamable", "streamable", "");
#else
#ifdef _USE_MEMORY_MANAGEMENT
	xal::manager->createCategory("cat", xal::BufferMode::Managed, SOURCE_MODE);
#else
	xal::manager->createCategory("cat", xal::BufferMode::Full, SOURCE_MODE);
#endif
	xal::manager->createSound(RESOURCE_PATH "linked/linked_sound.xln", "cat");
#endif
	xal::Player* player = xal::manager->createPlayer(USED_SOUND);
	...
	xal::manager->destroyPlayer(player);
	hlog::write("", "  - done");
	xal::destroy();
#ifndef _WINRT
	system("pause");
